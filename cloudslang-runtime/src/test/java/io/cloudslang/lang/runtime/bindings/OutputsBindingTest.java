...
public class OutputsBindingTest {
    ...
    public void testOperationEmptyOutputs() {
        ...
        Map<String, Value> result = outputsBinding.bindOutputs(operationContext, actionReturnValues, EMPTY_SET, outputs);
    ...
    public void testOperationOutputsNoExpression() {
        ...
        List<Output> outputs = Collections.singletonList(createNoExpressionOutput("host1"));
    ...
    public void testOperationOutputsNoExpressionAtAll() {
        Map<String, Value> operationContext = prepareOperationContext();
        Map<String, Value> actionReturnValues = new HashMap<>();
        List<Output> outputs = Collections.singletonList(createNoExpressionOutput("actionOutputKey1"));
    ...
    public void testOperationOutputsIllegalEvaluatedExpression() {
        Map<String, Value> operationContext = prepareOperationContext();
        Map<String, Value> actionReturnValues = new HashMap<>();
        List<Output> outputs = Collections.singletonList(createExpressionOutput("actionOutputKey1", "${ None + 'str' }"));
    ...
    public void testOperationOutputsInvalidExpression() {
        Map<String, Value> operationContext = prepareOperationContext();
        Map<String, Value> actionReturnValues = prepareActionReturnValues();
        List<Output> outputs = Collections.singletonList(createExpressionOutput("hostFromExpression", "${ 'http://' + hostExpr + ':' + str(self[SHOULD_BE_STRING]) }"));
